...
def bars_path_to_tabseq(bars_paths: List[List[Dict[str, Any]]], min_positions_per_bar: int = 8) -> TabSeq:
	"""Convert a list of per-bar HMM paths into a TabSeq with one Tab per bar.
	Each bar will have at least min_positions_per_bar columns (padded with empty positions)."""
	tabs: List[Tab] = []
	for segment in bars_paths:
		n_positions = max(len(segment), int(min_positions_per_bar))
		if n_positions <= 0:
			continue
		tab = Tab(n_positions=n_positions)
		for local_pos, form in enumerate(segment):
			fret_config = form.get('fret_config', {})
			for string_idx in range(6):
				fret = fret_config.get(5-string_idx, fret_config.get(str(5-string_idx), -1))
...
def export_hmm_bars(bars_paths: List[List[Dict[str, Any]]], song_name: str = 'arrangement', tempo: int = 90,
					 output_dir: str = 'outputs', sf2_path: str = 'resources/Tyros Nylon.sf2',
					 export_separate: bool = True, verbose: bool = True,
					 min_positions_per_bar: int = 8) -> Dict[str, str]:
	"""Export an arrangement given per-bar HMM paths (variable positions per bar).
	Ensures each bar has at least min_positions_per_bar columns for consistent tab width."""
	dest_dir = os.path.join(output_dir, song_name) if export_separate else output_dir
	ensure_dir(dest_dir)

	tabseq = bars_path_to_tabseq(bars_paths, min_positions_per_bar=min_positions_per_bar)
